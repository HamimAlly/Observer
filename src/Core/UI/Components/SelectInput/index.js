...
const CLASS_SELECT_INPUT = 'select-input';
const CLASS_ALLOW_INPUT = 'select-input-allow-input';
...
class SelectInput extends Element
{
    constructor(args = {})
    {
        ...
        super(Object.assign(Object.assign({}, args), { dom: container.dom }));
        ...
        this._onInputKeyDown = (evt) => {
            if (evt.key === 'Enter' && this.enabled && !this.readOnly)
            {
                ...
                evt.preventDefault();
                let value;
                if (this._labelHighlighted && this._labelToValue.has(this._labelHighlighted)) value = this._labelToValue.get(this._labelHighlighted);
                else value = this._input.value;

                if (value !== undefined)
                {
                    this.focus();
                    this.close();
                    if (this._valueToText[value]) this._onSelectValue(value);
                    
                    else if (this._allowCreate)
                    {
                        if (this._createFn) this._createFn(value);
                        else this._onSelectValue(value);
                    }
        ...
        this._onDocumentPointerDown = (evt) => {if (!this.dom.contains(evt.composedPath()[0])) this.close();};
        ...
        this._onKeyDown = (evt) => {
            ...
            if (this._containerOptions.hidden)
            {
                ...
                if (evt.key === 'ArrowUp') index--;
                else if (evt.key === 'ArrowDown') index++;

                if (index >= 0 && index < this._options.length) this._onSelectValue(this._options[index].v);
            }
            ...
            else
            {
                ...
                if (!this._labelHighlighted) this._highlightLabel(this._containerOptions.dom.childNodes[0].ui);
                ...
                else
                {
                    ...
                    do
                    {
                        if (evt.key === 'ArrowUp') highlightedLabelDom = highlightedLabelDom.previousSibling;
                        else if (evt.key === 'ArrowDown') highlightedLabelDom = highlightedLabelDom.nextSibling;
                    } 
                    
                    while (highlightedLabelDom && highlightedLabelDom.ui.hidden);
                    if (highlightedLabelDom) this._highlightLabel(highlightedLabelDom.ui);
                }
            }
        };

        this._onPointerDown = () => {if (!this._allowInput) this.focus();};

        this._onFocus = () => {
            this.class.add(CLASS_FOCUS);
            this.emit('focus');
            if (!this._input.hidden) this.open();
        };
        ...
        this._onWheel = (evt) => {evt.stopPropagation();};

        this._container = container;
        this._container.parent = this;
        this.class.add(CLASS_SELECT_INPUT);

        this._containerValue = new Container({class: CLASS_SELECT_INPUT_CONTAINER_VALUE});
        ...
        if (this._allowInput) this.class.add(CLASS_ALLOW_INPUT);

        this._allowCreate = args.allowCreate;
        this._createFn = args.createFn;
        this._createLabelText = args.createLabelText;

        this._labelValue = new Label({class: CLASS_VALUE, tabIndex: 0});
        this._labelValue.on('click', () => {if (this.enabled && !this.readOnly) this.toggle();});
        ...
        if (args.placeholder) this.placeholder = args.placeholder;

        this._containerOptions = new Container({class: CLASS_LIST, hidden: true});
        ...
        else if (args.defaultValue) this.value = args.defaultValue;
        else this.value = null;
        this._renderChanges = args.renderChanges;

        this.on('change', () => {
            this._updateInputFieldsVisibility();
            if (this.renderChanges && !this.multiSelect) this._labelValue.flash();
        });
    ...
    _initializeCreateLabel()
    {
        ...
        container.on('click', (e) => {
            ...
            if (this._createFn) this._createFn(text);
            else if (text) this._onSelectValue(text);
        });
        ...
        const labelCreateText = new Label({text: this._createLabelText});
    ...
    _convertSingleValue(value)
    {
        ...
        else if (this._type === 'boolean') return !!value;
        return value;
    }
    ...
    _onSelectValue(value)
    {
        ...
        if (this._values)
        {
            ...
            if (dirty)
            {
                ...
                if (this._binding) this._binding.addValues([value]);
            }
        }

        else
        {
            if (!this._value || !Array.isArray(this._value)) this.value = [value];
            else
            {
    ...
    _highlightLabel(label)
    {
        ...
        if (this._labelHighlighted) this._labelHighlighted.class.remove(CLASS_HIGHLIGHTED);
        ...
        if (this._labelHighlighted)
        {
            ...
            if (labelTop < scrollTop) this._containerOptions.dom.scrollTop = labelTop;
            else if (labelTop + this._labelHighlighted.height > this._containerOptions.height + scrollTop) this._containerOptions.dom.scrollTop = labelTop + this._labelHighlighted.height - this._containerOptions.height;
        }
    }
    ...
    _onValueChange(value)
    {
        if (!this.multiSelect)
        {
            ...
            for (const key in this._valueToLabel)
            {
                ...
                if (key === value) label.class.add(CLASS_SELECTED);
                else label.class.remove(CLASS_SELECTED);
            }
        }
        ...
        else
        {
            ...
            if (value && Array.isArray(value))
            {
                for (const val of value)
                {
                    ...
                    if (label) label.class.add(CLASS_SELECTED);
                }

                for (const key in this._valueToLabel)
                {
                    const label = this._valueToLabel[key];
                    if (value.indexOf(this._convertSingleValue(key)) !== -1) label.class.add(CLASS_SELECTED);
                    else label.class.remove(CLASS_SELECTED);
                }
            }
        }
    ...
    _onMultipleValuesChange(values)
    {
        ...
        for (const val in valueCounts)
        {
            if (valueCounts[val] !== values.length)
            {
                ...
                const label = this._valueToLabel[String(val)];
                if (label) label.class.remove(CLASS_SELECTED);
            }
        }
    }
    ...
    _addTag(value)
    {
        ...
        container.append(new Label({text: this._valueToText[String(value)] || value}));
    ...
    _removeTag(tagElement, value)
    {
        ...
        this.emit('change', this.value);
        if (this._binding) this._binding.removeValues([value]);
    }

    _filterOptions(filter)
    {
        const containerDom = this._containerOptions.dom;
        while (containerDom.firstChild) containerDom.removeChild(containerDom.lastChild);
        ...
        if (this._createLabelContainer) containerDom.appendChild(this._createLabelContainer.dom);
        if (containerDom.firstChild) this._highlightLabel(containerDom.firstChild.ui);

        this._resizeShadow();
    }

    _resizeShadow() {this._domShadow.style.height = `${this._containerValue.height + this._containerOptions.height}px`;}
    ...
    _updateInputFieldsVisibility(focused)
    {
        ...
        if (this._allowInput)
        {
            ...
            else showInput = this.multiSelect || !this._valueToLabel[this.value];
        }
    ...
    focus()
    {
        if (this._input.hidden) this._labelValue.dom.focus();
        else this._input.focus();
    }

    blur()
    {
        if (this._allowInput) this._input.blur();
        else this._labelValue.dom.blur();
    }
    ...
    open()
    {
        ...
        if (this._optionsFn) this.options = this._optionsFn();
        if (this._containerOptions.dom.childNodes.length === 0) return;

        this._containerOptions.forEachChild((label) => {
            label.hidden = false;
            if (this._labelToValue.get(label) === this.value) this._highlightLabel(label);
        });
        ...
        if (fitHeight) this.class.add(CLASS_FIT_HEIGHT);
        else this.class.remove(CLASS_FIT_HEIGHT);
    ...
    toggle()
    {
        if (this._containerOptions.hidden) this.open();
        else this.close();
    }
    ...
    {
        super.unlink();
        if (!this._containerOptions.hidden) this.close();
    }
    ...
    _updateValue(value)
    {
        ...
        if (!this._suppressChange) this.emit('change', value);
        if (this._binding) this._binding.setValue(value);
    }
    ...
    set options(value)
    {
        ...
        if (this._createLabelText) this._createLabelContainer = this._initializeCreateLabel();
        
        if (this.multiSelect && this._values) this._onMultipleValuesChange(this._values);
        else this._onValueChange(this.value);

        if (this._lastInputValue) this._filterOptions(this._lastInputValue);
    }

    get options() {return this._options.slice();}

    set invalidOptions(value) {this._invalidOptions = value || null;}
    get invalidOptions() {return this._invalidOptions;}
    
    set disabledValue(value)
    {
        this._disabledValue = value;
        if (this._disabledValue !== null) this.class.add(CLASS_HAS_DISABLED_VALUE);
        else this.class.remove(CLASS_HAS_DISABLED_VALUE);
    }
    ...
    set fallbackOrder(value) {this._fallbackOrder = value || null;}
    get multiSelect() {return this.class.contains(CLASS_MULTI_SELECT);}
    ...
    set value(value)
    {
        ...
        {
            if (value !== null || !this._allowNull || !this.class.contains(CLASS_MULTIPLE_VALUES)) return;
        }
    ...
    get value()
    {
        if (!this.multiSelect) return this._value;
        const result = [];
        this._containerTags.dom.childNodes.forEach((dom) => {result.push(dom.ui.value);});
        return result;
    }

    set values(values)
    {
        values = values.map((value) => {return this._convertValue(value);});
        let different = false;
        ...
        else this.value = values[0];
    }

    set placeholder(value) {this._input.placeholder = value;}
